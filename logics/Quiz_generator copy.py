...
category_n_course_name = df.groupby('category')['name'].apply(list).to_dict()
...
def identify_category_and_courses(user_message):
    ...
    print(category_and_product_response_str)
...
def get_course_details(list_of_relevant_category_n_course: list[dict]):
    ...
    #print('list_of_course_details :', list_of_course_details)
...
def generate_response_based_on_course_details(user_message, product_details):
    ...
    print('response_to_customer 1 : ', response_to_customer)

    if response_to_customer.split(delimiter)[-1]:
        response_to_customer = response_to_customer.split(delimiter)[-1]
    else :
         response_to_customer = response_to_customer.split(delimiter)[-2]

    #print('response_to_customer 2 : ', response_to_customer)
...
def generate_mcq(user_message, num_questions=10):
    ...
    3. Generate exactly {num_questions} multiple-choice questions.
    ...
    7. Output only JSON in the following format:
        [
            {
                "Question no": 1,
                "Question": "What is correct based on the document? (1)",
